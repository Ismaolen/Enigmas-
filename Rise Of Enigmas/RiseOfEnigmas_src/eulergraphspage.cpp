...
void EulerGraphsPage::checkLevelScore(){
    QString reachedLevels;
        if(!solvedLevels.empty()){
          reachedLevels =  "You have reached the following levels: \n";
        }
        QString levelsNotYetReached = "The following levels are not yet reached\n";
        std::vector <uint>  correctSolvedLevels = {0, 0, 0, 0, 0, 0, 0, 0}; // Vektor Zum Speichern von erfolgreich abgeschlossenen Graphen
        uint increment = 0;
        // Schleife zum Ersetzen der Nullen in correctSolvedLevels-Vektor durch die in solvedLevels-Verktor gespeicherten Levels, Zum Anzeigen der abgeschlossenen Levels
        if(!solvedLevels.empty()){
        for(uint i = 0; i < correctSolvedLevels.size(); i++){
            if(i == solvedLevels[increment] ){
                correctSolvedLevels[i-1] = i;
                i = 0;
                if(increment < solvedLevels.size()-1){
                    increment++;
                }
                else break;
            }
        ...
        // Zum Anzeigen der noch nicht abgeschlossenen Levels
        std::vector <uint> levelsToBeReached = {1, 2, 3, 4, 5, 6, 7, 8}; // hier werden die Levels gespeichert um diese mit den abgeschlossenen Levels zu vergleichen.
        // Schleife zum Speichern der abgeschlossenen und nicht abgeschlossenen Levels als String, der durch QMessageBox ausgegeben wird
        for(uint i = 0; i < 8; i++){
            if(correctSolvedLevels[i] == levelsToBeReached[i]){
                levelsToBeReached[i] = 0;
                reachedLevels +=  ""+ QString::number(correctSolvedLevels[i]) + "\n";
            }
            if(levelsToBeReached[i] != 0){
                levelsNotYetReached += " "+QString::number(levelsToBeReached[i]) + "\n";
            }
        }
        QMessageBox::information(this, tr("Erreichte Levels"),
        reachedLevels+ levelsNotYetReached);
}

void EulerGraphsPage::setNextEulerGraph()
{
    currentGraphNum = eulerGraph->m_scene.m_currentStage;
    solvedLevels.push_back(currentGraphNum); // Das aktuell gelöste Level zum Ende des Vektors hinzufügen
    sort(solvedLevels.begin(), solvedLevels.end()); // Vectorsortierung
    solvedLevels.erase( unique( solvedLevels.begin(), solvedLevels.end() ), solvedLevels.end() ); // doppelte Elemente löschen. Um abgeschlossene Levels einmal zu speichern.
...
{
    rightButton->setEnabled(currentGraphNum < 7 );
...
{
    if(currentGraphNum >= 7)
    {
        rightButton->setEnabled(false);// Buttondeaktivierung, falls der aktuelle Graph, der letzte Graph ist.
        return;
    }
    ++currentGraphNum;
    refreshShow();
